...
const summaryDocRef = doc(db, 'thai-student-accountSummary', 'latest');
const transactionsCollectionRef = collection(db, 'thai-student-transactions');
...
export const listenToThaiStudentAccountSummary = (callback: (summary: AccountSummary | null) => void) => {
    ensureInitialState();
...
export const updateThaiStudentAccountSummary = async (summary: Partial<Omit<AccountSummary, 'id'>>) => {
    await setDoc(summaryDocRef, summary, { merge: true });
};

export const listenToThaiStudentTransactions = (
    ...
    (error) => {
        console.error("Error in thai student account transaction listener:", error);
        if (onError) {
            onError(error);
        }
    });
...
export const addThaiStudentTransaction = async (transaction: Omit<Transaction, 'id' | 'businessType'>) => {
    const newTransactionRef = doc(transactionsCollectionRef);
    await setDoc(newTransactionRef, { 
        ...transaction,
        businessType: 'thai-student',
        date: Timestamp.fromDate(transaction.date),
        createdAt: serverTimestamp()
    });
};

export const updateThaiStudentTransaction = async (id: string, updatedFields: Partial<Omit<Transaction, 'id'>>) => {
...
export const deleteThaiStudentTransaction = async (id: string) => {
    const transactionDocRef = doc(transactionsCollectionRef, id);
