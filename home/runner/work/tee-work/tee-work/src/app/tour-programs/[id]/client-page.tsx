...
import { ArrowLeft, Save, Trash2, MapPin, Calendar as CalendarIcon, BedDouble, Truck, Plane, TrainFront, PlusCircle, Camera, UtensilsCrossed, Users, FileText, Clock, Eye, EyeOff, Printer, Earth, Ticket, FilePieChart, BookOpen, TrendingUp, Handshake, CalendarClock } from "lucide-react";
...
export default function TourProgramClientPage({ initialCalculation }: { initialCalculation: any }) {
    ...
    useEffect(() => {
        if (initialCalculation) {
            setLocalProgram(initialCalculation);
            setExchangeRates(initialCalculation.exchangeRates || initialRates);
            setLoading(false);
        } else if (localProgram?.id) {
             setLoading(true);
             const fetchProgram = async () => {
                 try {
                    const fetchedProgram = await new Promise<TourProgram | null>((resolve) => setTimeout(() => resolve(initialCalculation), 1000));
                    if (fetchedProgram) {
    ...
    const addDividendRow = () => {
        setDividendStructure(prev => [...prev, { id: uuidv4(), name: '', percentage: 0 }]);
    };
    ...
    const totalPercentage = useMemo(() => {
        return dividendStructure.reduce((sum, item) => sum + item.percentage, 0);
    }, [dividendStructure]);
