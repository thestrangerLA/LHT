...
export default function LoanDetailPage() {
  ...
  useEffect(() => {
    ...
    const unsubLoan = listenToLoan(id, async (loanData) => {
        if (loanData) {
            setLoan(loanData);
    ...
    const unsubRepay = listenToRepaymentsForLoan(id, setRepayments);
  ...
  /* ---------------- summary ---------------- */
    ...
    let runningBalance: Omit<CurrencyValues, 'cny'> = { ...(loan.repaymentAmount || { kip: 0, thb: 0, usd: 0 }) };
    
    const sortedRepayments = [...repayments].sort((a, b) => toDateSafe(a.repaymentDate)!.getTime() - toDateSafe(b.repaymentDate)!.getTime());
    ...
    return repayments.map(r => historyMap.get(r.id)).filter(Boolean).sort((a,b) => toDateSafe(b.repaymentDate)!.getTime() - toDateSafe(a.repaymentDate)!.getTime());
  }, [loan, repayments]);
                ...
                <h3 className="text-lg font-semibold">ປະຫວັດການຊຳລະ: {loan.memberId ? member?.name : loan.debtorName || '...'}</h3>
                 <Table>
                    ...
                    <TableRow>
                        ...
                        <TableHead className="text-right">ເງິນຕົ້ນທີ່ຈ່າຍ</TableHead>
                        <TableHead className="text-right">ກຳໄລທີ່ຈ່າຍ</TableHead>
                        ...
                        <TableCell className="text-right">
                         ...
                         <TableCell className="text-right">
                            ...
                            (c) =>
                                ...
                                <div key={c}>
                                    {formatCurrency(r.principalPortion?.[c] ?? 0)}{" "}
                                    {c.toUpperCase()}
                                </div>
                                )
                            )}
                         ...
                         <TableCell className="text-right text-blue-500">
                            ...
                            (c) =>
                                ...
                                <div key={c}>
                                    {formatCurrency(r.profitPortion?.[c] ?? 0)}{" "}
                                    {c.toUpperCase()}
                                </div>
                                )
                            )}
